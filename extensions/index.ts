...
function buildNotifySeq(
  protocol: Exclude<NotifyProtocol, "auto">,
  title: string,
  body: string
): string {
...
function sendNotification(
  ...
  let seq = buildNotifySeq(
    protocol === "auto" ? detectProtocol() : protocol,
    title,
    body
  )
  if (process.env.TMUX) {
...
export default function (pi: ExtensionAPI) {
  ...
  pi.registerCommand("pi-tmux-notify", {
    description:
      "Toggle finish notifications: on|off|osc777|osc99|osc9|auto|test",
    handler: async (args, ctx) => {
      ...
      if (arg === "on") enabled = true
        ...
        sendNotification(protocol, "pi test", "Test notification")
      else enabled = !enabled
      if (!enabled) await disarmFocusHook()
      const effective =
        protocol === "auto" ? `auto (${detectProtocol()})` : protocol
      ctx.ui.notify(
        `pi-tmux-notify: ${enabled ? "on" : "off"}, protocol: ${effective}`,
        "info"
      )
    },
  })
